...
后记：前段时间看《Java并发编程的艺术》这本书，发现书中对`volatile`关键字的介绍有些深入不浅出(概念抽象不太到位，要么具体到无关紧要的细节，要么太笼统一带而过)。后来在网上找相关资料时发现了这个并发编程系列的博客，一读之后豁然开朗，于是便有了翻译过来与各位共享的想法。后续还会继续翻译一些该系列中的博客。
